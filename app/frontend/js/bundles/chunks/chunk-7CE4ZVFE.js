(function(){"use strict";let Z=["Aries","Taurus","Gemini","Cancer","Leo","Virgo","Libra","Scorpio","Sagittarius","Capricorn","Aquarius","Pisces"],O=["Sun","Moon","Mercury","Venus","Mars","Jupiter","Saturn","Uranus","Neptune","Pluto","Chiron","Proserpina"],ge=O.slice(0,10),ne="dispositorChainDisplayOptions",I={mode:"domicile",showArrowDirection:!0,showHouseRulers:!0,classicalRulers:!1},ae={Aries:"Mars",Taurus:"Venus",Gemini:"Mercury",Cancer:"Moon",Leo:"Sun",Virgo:"Mercury",Libra:"Venus",Scorpio:"Mars",Sagittarius:"Jupiter",Capricorn:"Saturn",Aquarius:"Saturn",Pisces:"Jupiter"},ie=Object.fromEntries(Z.map((e,s)=>[e,Z[(s+6)%12]])),fe={Aries:{ruler:"Mars",co_ruler:null,exaltation:"Sun"},Taurus:{ruler:"Venus",co_ruler:null,exaltation:"Moon"},Gemini:{ruler:"Mercury",co_ruler:null,exaltation:null},Cancer:{ruler:"Moon",co_ruler:null,exaltation:"Jupiter"},Leo:{ruler:"Sun",co_ruler:null,exaltation:null},Virgo:{ruler:"Mercury",co_ruler:"Proserpina",exaltation:"Mercury"},Libra:{ruler:"Venus",co_ruler:"Chiron",exaltation:"Saturn"},Scorpio:{ruler:"Pluto",co_ruler:"Mars",exaltation:null},Sagittarius:{ruler:"Jupiter",co_ruler:"Neptune",exaltation:null},Capricorn:{ruler:"Saturn",co_ruler:"Uranus",exaltation:"Mars"},Aquarius:{ruler:"Uranus",co_ruler:"Saturn",exaltation:null},Pisces:{ruler:"Neptune",co_ruler:"Jupiter",exaltation:"Venus"}},ye={TrueNorthNode:"TrueNode",TrueSouthNode:"SouthNode",Fortune:"PartOfFortune"};function f(e,s){return window.FrontendI18n?.t?.(e,s)||e}function h(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function G(e){return ye[e]||e}function D(e){if(!e)return"—";let s=`astro.planet.${e}`,t=f(s);return t!==s?t:window.Symbols?.getPlanetNameRu?.(e)||window.Symbols?.planetNamesRu?.[e]||e}function be(e){if(!e)return"—";let s=`astro.sign.${e}`,t=f(s);return t!==s?t:window.Symbols?.signNamesRu?.[e]||e}function J(e,s=16){return window.Symbols?.getPlanetSymbolMarkup?.(e,{size:s,title:D(e)})||`<span class="astro-symbol">${h(window.Symbols?.getPlanetSymbol?.(e)||"")}</span>`}function le(e){return[window.Symbols?.signs?.[e]||"",be(e)].filter(Boolean).join(" ")}function Q(){let e={},s=window.accountPreferencesCache?.methodology?.dignities?.signs||window.accountPreferencesCache?.methodology?.default_dignities?.signs||{};return Z.forEach(t=>{let o=fe[t]||{},a=s?.[t]||{},l=G(a.ruler||o.ruler||null),S=G(a.co_ruler||o.co_ruler||null),m=G(a.exaltation||o.exaltation||null);l&&S&&l===S&&(S=null),e[t]={ruler:l,co_ruler:S,exaltation:m}}),e}function U(e,s,t=Q()){let o=t?.[e]||{},a=t?.[ie[e]]||{};return s==="exaltation"?o.exaltation||null:s==="detriment"?a.ruler||null:s==="fall"?a.exaltation||null:o.ruler||null}function ce(e,s,t,o=I){return e?o.classicalRulers&&s==="domicile"?ae[e]||U(e,s,t):o.classicalRulers&&s==="detriment"&&ae[ie[e]]||U(e,s,t):null}function pe(e){return(Array.isArray(e?.planets)?e.planets:[]).filter(t=>t?.name&&t?.sign&&O.includes(G(t.name))).map(t=>({...t,name:G(t.name)})).sort((t,o)=>O.indexOf(t.name)-O.indexOf(o.name))}function de(e,s){let t=Q(),o=pe(e),a=new Map(o.map(p=>[p.name,p])),l=[],S=new Map;o.forEach(p=>{let d=[],i=new Map,c=p,y=null,x=[];for(;c?.name&&!i.has(c.name);){i.set(c.name,d.length);let g=U(c.sign,s,t);if(d.push({planet:c.name,sign:c.sign,ruler:g,retrograde:!!c.retrograde}),!g){y="none";break}if(!a.has(g)){y=g;break}if(g===c.name){y=g;break}c=a.get(g)}if(!y&&c?.name&&i.has(c.name)){let g=i.get(c.name);x=d.slice(g).map(_=>_.planet),y=x.join("+")}S.set(y,(S.get(y)||0)+1),l.push({start:p.name,steps:d,finalKey:y,cycle:x})});let m=[...S.entries()].filter(([p])=>p&&p!=="none").sort((p,d)=>d[1]-p[1]||p[0].localeCompare(d[0])).slice(0,4);return{chains:l,mainRulers:m}}function ee(e){if(!e)return`<p class="dispositor-empty">${h(f("page.chart.rulers.empty.noJones"))}</p>`;let s=(()=>{let o=`astro.pattern.${e.pattern_type}`,a=f(o);return a===o?e.pattern_type||"—":a})(),t=[];return Number.isFinite(Number(e.empty_arc_degree))&&t.push(f("page.chart.balances.emptyArc",{value:Number(e.empty_arc_degree).toFixed(0)})),e.handle_planet&&t.push(f("page.chart.balances.handle",{planet:D(e.handle_planet)})),e.leading_planet&&t.push(f("page.chart.balances.leading",{planet:D(e.leading_planet)})),`
        ...
        `}function T(e){return[...new Set(e)].sort((s,t)=>{let o=O.indexOf(s),a=O.indexOf(t);return(o===-1?999:o)-(a===-1?999:a)})}function we(e){return T(e).join("+")}function Se(e){let s=e?.number??e?.house_number,t=Number(s);return Number.isInteger(t)?t:s}function xe(e){let s=[...new Set(e)].map(t=>Number(t)).filter(t=>Number.isInteger(t)).sort((t,o)=>t-o);return s.length?window.Symbols?.formatHouseList?.(s,{style:"roman",separator:","})||s.map(t=>["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII"][t-1]||String(t)).join(","):""}function Me(e,s,t,o=I){return s==="domicile"&&e?.ruler_planet&&!o.classicalRulers?G(e.ruler_planet):G(ce(e?.sign,s,t,o))}function ve(e,s,t=I){let o=Q(),a=pe(e),l=new Map(a.map(i=>[i.name,i])),S=Array.isArray(e?.houses)?e.houses:[],m=new Map,p=[];return S.forEach(i=>{let c=Me(i,s,o,t),y=Se(i);!c||!y||(m.has(c)||m.set(c,[]),m.get(c).push(y),p.push(c))}),a.forEach(i=>{ge.includes(i.name)&&p.push(i.name)}),{chains:T(p).map(i=>{let c=[],y=new Map,x=l.get(i)||{name:i,sign:null,retrograde:!1},g=null,_=[];for(;x?.name&&!y.has(x.name);){y.set(x.name,c.length);let v=x.sign?ce(x.sign,s,o,t):null;if(c.push({planet:x.name,sign:x.sign,ruler:v,retrograde:!!x.retrograde}),!v){g=x.name;break}if(!l.has(v)){g=v;break}if(v===x.name){g=v;break}x=l.get(v)}if(!g&&x?.name&&y.has(x.name)){let v=y.get(x.name);_=c.slice(v).map(P=>P.planet),g=we(_)}return{start:i,steps:c,finalKey:g,cycle:_}}),housesByRuler:m}}function Ee(e,s,t,o=""){let a=t.showHouseRulers?xe(s.get(e.planet)||[]):"",l=[D(e.planet),e.sign?le(e.sign):"",a?`${f("common.house")} ${a}`:""].filter(Boolean).join(" · ");return`
            <span
        ...
        `}function ke(e,s,t){let o=[],a=new Set;if(e.forEach(m=>{let p=m.steps.map(d=>d.planet).join(">");a.has(p)||(a.add(p),o.push(m))}),!o.length)return`<p class="dispositor-empty">${h(f("page.chart.rulers.empty.noChains"))}</p>`;let l=new Map;return o.forEach(m=>{let p=m.finalKey||"none";l.has(p)||l.set(p,[]),l.get(p).push(m)}),`
            <div class="dispositor-compact-diagram">
                ${[...l.entries()].sort((m,p)=>{let d=new Set(m[1].flatMap(c=>c.steps.map(y=>y.planet))).size,i=new Set(p[1].flatMap(c=>c.steps.map(y=>y.planet))).size;return d-i||String(m[0]).localeCompare(String(p[0]))}).map(([m,p],d)=>{let i=Re(m,p),c=`url(#dispositorCompactArrow${d})`,y=t.showArrowDirection?` marker-end="${c}"`:"",x=t.showArrowDirection?` marker-start="${c}" marker-end="${c}"`:"";return`
        ...
        `}function Re(e,s){let p=e&&e!=="none"?e.split("+").filter(Boolean):[],d=new Map,i=[],c=new Set,y=(r,u={})=>{if(!r)return null;let $=d.get(r)||{planet:r,sign:null,retrograde:!1};return d.set(r,{...$,sign:$.sign||u.sign||null,retrograde:$.retrograde||!!u.retrograde}),d.get(r)},x=(r,u)=>{let $=r?.planet,M=u?.planet;if(!$||!M||$===M)return;y($,r),y(M,u);let R=`${$}->${M}`;c.has(R)||(c.add(R),i.push({child:$,parent:M}))};s.forEach(r=>{r.steps.forEach(u=>y(u.planet,u));for(let u=0;u<r.steps.length;u+=1){let $=r.steps[u],M=r.steps[u+1];M?x($,M):$?.ruler&&!r.steps.some(R=>R.planet===$.ruler)&&x($,{planet:$.ruler})}});let g=p.length?T(p):T([...d.keys()].filter(r=>!i.some(u=>u.child===r)));!g.length&&d.size&&g.push([...d.keys()][0]);let _=new Set(g),v=new Map,P=[],q=[];i.forEach(r=>{if(_.has(r.child)&&_.has(r.parent)){let u=T([r.child,r.parent]).join("<->");P.some($=>$.key===u)||P.push({...r,key:u});return}q.push(r),v.has(r.parent)||v.set(r.parent,[]),v.get(r.parent).push(r.child)}),v.forEach((r,u)=>{v.set(u,T(r))});let k=new Map,z=(r,u,$)=>{let M=8,R=(A,H=0,C=new Set)=>{if(k.has(A))return k.get(A);if(C.has(A)){let j={x:$+u*H*44,y:M};return M+=54,k.set(A,j),j}C.add(A);let Y=(v.get(A)||[]).filter(j=>!_.has(j)),B;if(!Y.length)B=M,M+=54;else{let j=Y.map(F=>R(F,H+1,new Set(C)));B=(Math.min(...j.map(F=>F.y))+Math.max(...j.map(F=>F.y)))/2}C.delete(A);let X={x:$+u*H*44,y:B};return k.set(A,X),X};return{rootPosition:R(r,0),height:M}};if(g.length===2){let r=g[0],u=g[1],$=z(r,-1,0),M=z(u,1,46),R=Math.max($.rootPosition.y,M.rootPosition.y),N=(A,H)=>{let C=[A],Y=new Set;for(;C.length;){let B=C.pop();if(!B||Y.has(B))continue;Y.add(B);let X=k.get(B);X&&(X.y+=H),(v.get(B)||[]).forEach(j=>{_.has(j)||C.push(j)})}};N(r,R-$.rootPosition.y),N(u,R-M.rootPosition.y)}else g.forEach((r,u)=>{let $=z(r,-1,u*108),M=u===0?0:u*108-$.rootPosition.y;M&&k.forEach((R,N)=>{(N===r||(v.get(r)||[]).includes(N))&&(R.y+=M)})});d.forEach((r,u)=>{k.has(u)||k.set(u,{x:0,y:8+k.size*54})});let oe=Math.min(...[...k.values()].map(r=>r.x)),re=Math.min(...[...k.values()].map(r=>r.y));k.forEach(r=>{r.x=r.x-oe+8,r.y=r.y-re+8});let V=[...d.values()].map(r=>({...r,isRoot:_.has(r.planet),...k.get(r.planet)||{x:8,y:8}})),K=new Map(V.map(r=>[r.planet,r])),n=r=>{let u=K.get(r.child),$=K.get(r.parent);if(!u||!$)return null;let M=5,R=u.x<$.x,N=R?u.x+34+M:u.x-M,A=R?$.x-M:$.x+34+M,H=u.y+18,C=$.y+18,Y=N+(A-N)/2;return{...r,path:`M${N},${H} H${Y} V${C} H${A}`}},w=q.map(n).filter(Boolean),b=P.map(n).filter(Boolean),E=Math.max(220,Math.ceil(Math.max(...V.map(r=>r.x+34))+8)),L=Math.max(70,Math.ceil(Math.max(...V.map(r=>r.y+44))+8));return{width:E,height:L,nodes:V,edges:w,mutualEdges:b}}function _e(e){let s=[],t=new Set;e.forEach(a=>{let l=a.steps.map(S=>S.planet).join(">");t.has(l)||(t.add(l),s.push(a))});let o=new Map;return s.forEach(a=>{let l=a.finalKey||"none";o.has(l)||o.set(l,[]),o.get(l).push(a)}),s.length?`
        ...
        `:`<p class="dispositor-empty">${h(f("page.chart.rulers.empty.noChains"))}</p>`}function Le(e,s){let S=new Set(e&&e!=="none"?e.split("+").filter(Boolean):[]),m=new Map,p=[],d=new Set,i=new Map,c=new Map,y=(n,w={})=>{if(!n)return null;let b=m.get(n)||{planet:n,sign:null,retrograde:!1};return m.set(n,{...b,sign:b.sign||w.sign||null,retrograde:b.retrograde||!!w.retrograde}),m.get(n)},x=(n,w)=>{let b=n?.planet,E=w?.planet;if(!b||!E||b===E||S.has(b)&&S.has(E))return;y(b,n),y(E,w);let L=`${b}->${E}`;d.has(L)||(d.add(L),p.push({child:b,parent:E}),c.set(b,E),i.has(E)||i.set(E,[]),i.get(E).push(b))};s.forEach(n=>{n.steps.forEach(b=>y(b.planet,b));for(let b=0;b<n.steps.length-1;b+=1)x(n.steps[b],n.steps[b+1]);let w=n.steps[n.steps.length-1];w?.ruler&&!n.steps.some(b=>b.planet===w.ruler)&&x(w,{planet:w.ruler})}),S.size||[...m.keys()].forEach(n=>{c.has(n)||S.add(n)}),!S.size&&m.size&&S.add([...m.keys()][0]),i.forEach((n,w)=>{i.set(w,T(n))});let g=new Map,_=(n,w=0)=>{g.has(n)&&g.get(n)<=w||(g.set(n,w),(i.get(n)||[]).forEach(b=>_(b,w+1)))};T([...S]).forEach(n=>_(n,0)),m.forEach((n,w)=>{g.has(w)||g.set(w,0)});let v=24,P=new Map,q=(n,w=new Set)=>{if(P.has(n))return P.get(n);if(w.has(n)){let L=v;return v+=76,P.set(n,L),L}w.add(n);let b=i.get(n)||[],E;if(!b.length)E=v,v+=76;else{let L=b.map(r=>q(r,new Set(w)));E=(Math.min(...L)+Math.max(...L))/2}return w.delete(n),P.set(n,E),E};T([...S]).forEach(n=>q(n)),m.forEach((n,w)=>q(w));let k=[...m.values()].map(n=>({...n,isRoot:S.has(n.planet),x:24+(g.get(n.planet)||0)*128,y:P.get(n.planet)||24})),z=new Map(k.map(n=>[n.planet,n])),oe=Math.max(0,...k.map(n=>g.get(n.planet)||0)),re=Math.max(180,v+24),V=Math.max(520,48+oe*128+44),K=p.map(n=>{let w=z.get(n.child),b=z.get(n.parent);if(!w||!b)return null;let E=w.x,L=w.y+44/2,r=b.x+44,u=b.y+44/2,$=Math.max(r+18,E-42);return{...n,path:`M${E},${L} H${$} V${u} H${r}`}}).filter(Boolean);return{width:V,height:re,nodes:k,edges:K}}function Pe(e,s){if(!e||e==="none")return`
        ...
        `}function ue(e){return["domicile","exaltation","detriment","fall"].includes(e)?e:I.mode}function Ce(e={}){let s={};try{s=JSON.parse(window.localStorage?.getItem(ne)||"{}")||{}}catch{s={}}return{...I,mode:ue(e.mode||s.mode||I.mode),showArrowDirection:(e.showArrowDirection??s.showArrowDirection??I.showArrowDirection)!==!1,showHouseRulers:(e.showHouseRulers??s.showHouseRulers??I.showHouseRulers)!==!1,classicalRulers:(e.classicalRulers??s.classicalRulers??I.classicalRulers)===!0}}function je(e){try{window.localStorage?.setItem(ne,JSON.stringify(e))}catch{}}function he(e){let s=`page.chart.rulers.chainModes.${e}`,t=f(s);return t!==s?t:f(e==="domicile"?"page.chart.rulers.chainModes.domicile":`astro.dignity.${e}`)}function Ie(e){let s=["domicile","exaltation","detriment","fall"];return`
        ...
        `}function Be(e,s={},t="domicile"){W();let{chains:o,mainRulers:a}=de(e,t),l=document.createElement("div");l.className="dispositor-modal-overlay",l.innerHTML=`
