import{a as T}from"./chunks/chunk-6WUMBF2I.js";import"./chunks/chunk-N2I5AW2C.js";import{a as I}from"./chunks/chunk-SYGUD2QR.js";import{a as b,b as E,c as L}from"./chunks/chunk-HX7TNMQQ.js";import{b as y}from"./chunks/chunk-IZUYVIPG.js";var Z=y(b()),U=y(E()),J=y(L());var V=y(I()),G=y(T());var d=window.location.hostname==="localhost"?"http://localhost:8000/api/v1":"/api/v1",k=["client_question","life_context","astrological_focus","guidance_given","decision_or_plan","follow_up","sensitive"],c={sessionId:null,personId:null,cs:null,reportDirty:!1};function s(t,e){return window.FrontendI18n?.t?.(t,e)||t}function x(t={}){return window.AstroAPI?.withLocaleHeaders?window.AstroAPI.withLocaleHeaders(t):t}function u(t,e={}){return fetch(t,{credentials:"include",...e,headers:x(e.headers||{})})}function n(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function $(t){if(!t)return"";let e=new Date(t);return Number.isNaN(e.getTime())?String(t):window.LocaleFormatters?.formatDateTime?window.LocaleFormatters.formatDateTime(e):e.toLocaleString()}function p(t,e){if(!e)return"";let o=`page.consultation.${t}.${e}`,a=s(o);return a===o?e:a}var h=null;function l(t,e=""){let o=document.getElementById("toast");o&&(o.textContent=t,o.className=`toast visible ${e}`,clearTimeout(h),h=setTimeout(()=>{o.className="toast"},2600))}function B(){let t=window.location.pathname.match(/\/consultation\/([^/?#]+)/);return t?decodeURIComponent(t[1]):null}document.addEventListener("DOMContentLoaded",P);async function P(){c.sessionId=B();let t=document.getElementById("backLink");if(t&&t.addEventListener("click",e=>{e.preventDefault(),A()}),window.addEventListener("beforeunload",e=>{c.reportDirty&&(e.preventDefault(),e.returnValue="")}),!c.sessionId){w(s("page.consultation.notFound"));return}if(window.AstroAPI?.getAccountPreferences)try{window.accountPreferencesCache=await window.AstroAPI.getAccountPreferences(),window.AstroPreferences?.setAccountVisualPreferences?.(window.accountPreferencesCache?.visual||{})}catch(e){console.warn("Consultation account preferences fallback to defaults:",e)}await H(),document.getElementById("pageLoader")?.classList.add("hidden")}function A(){c.personId?window.location.href=`/client/${encodeURIComponent(c.personId)}`:window.history.back()}var S=["en","uk","ru"];async function H(){try{let t=await u(`${d}/call-sessions/${c.sessionId}`);if(t.status===404){w(s("page.consultation.notFound"));return}if(!t.ok)throw new Error(`HTTP ${t.status}`);c.cs=await t.json(),c.personId=c.cs.person_id;let e=c.cs.summary_json?.session?.language;if(e&&S.includes(e)&&window.FrontendI18n?.setLocale)try{await window.FrontendI18n.setLocale(e,{persist:!1,source:"consultation-language"})}catch{}C()}catch(t){w(t.message)}}function w(t){document.getElementById("pageLoader")?.classList.add("hidden"),document.getElementById("consultRoot").innerHTML=`<div class="consult-card"><p class="consult-empty">${n(t)}</p></div>`}function C(){let t=c.cs,e=t.summary_json,o=document.getElementById("consultRoot"),a=t.started_at?$(`${t.started_at}Z`):t.created_at?$(`${t.created_at}Z`):"",r="",i=(e?.session?.consultation_types||[]).map(_=>`<span class="consult-type-chip">${n(p("consultationType",_))}</span>`).join(""),f=`${a}${t.client_name?` · ${t.client_name}`:""}`;r+=`<div class="consult-head">
    ...
    </div>`;let m="",v="";t.call_status==="summary_failed"?m+=`<div class="consult-card consult-card-status">
            <p class="consult-empty">${n(s("page.consultation.summaryFailed"))}</p>
            ${t.summary_error?`<p class="consult-empty">${n(t.summary_error)}</p>`:""}
        </div>`:e?(m+=D(e),m+=M(t,e),v+=R(e.key_points||[]),v+=j(e.open_questions_or_unclear_items||[])):m+=q(t),v+=`<div class="consult-card consult-card-memory">
    ...
    </div>`}function R(t){if(!t.length)return"";let e={};t.forEach(a=>{(e[a.category]=e[a.category]||[]).push(a)});let o="";return k.forEach(a=>{if(!e[a])return;let r=e[a].map(i=>`<li>${n(i.text)}<span class="consult-kp-by">${n(p("mentionedBy",i.mentioned_by))}</span></li>`).join("");o+=`<div class="consult-kp-group">
