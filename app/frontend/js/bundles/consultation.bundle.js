import"./chunks/chunk-G5PKYDTN.js";import{a as k}from"./chunks/chunk-6WUMBF2I.js";import{a as I}from"./chunks/chunk-2JFYRFI2.js";import{a as b,b as E,c as L}from"./chunks/chunk-6WTMXHVS.js";import{b as y}from"./chunks/chunk-IZUYVIPG.js";var Z=y(b()),U=y(E()),J=y(L());var V=y(I()),z=y(k());var d=window.location.hostname==="localhost"?"http://localhost:8000/api/v1":"/api/v1",x=["client_question","life_context","astrological_focus","guidance_given","decision_or_plan","follow_up","sensitive"],i={sessionId:null,personId:null,cs:null,reportDirty:!1};function s(t,e){return window.FrontendI18n?.t?.(t,e)||t}function T(t={}){return window.AstroAPI?.withLocaleHeaders?window.AstroAPI.withLocaleHeaders(t):t}function u(t,e={}){return fetch(t,{credentials:"include",...e,headers:T(e.headers||{})})}function n(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function $(t){if(!t)return"";let e=new Date(t);return Number.isNaN(e.getTime())?String(t):window.LocaleFormatters?.formatDateTime?window.LocaleFormatters.formatDateTime(e):e.toLocaleString()}function p(t,e){if(!e)return"";let a=`page.consultation.${t}.${e}`,o=s(a);return o===a?e:o}var w=null;function l(t,e=""){let a=document.getElementById("toast");a&&(a.textContent=t,a.className=`toast visible ${e}`,clearTimeout(w),w=setTimeout(()=>{a.className="toast"},2600))}function B(){let t=window.location.pathname.match(/\/consultation\/([^/?#]+)/);return t?decodeURIComponent(t[1]):null}document.addEventListener("DOMContentLoaded",A);async function A(){i.sessionId=B();let t=document.getElementById("backLink");if(t&&t.addEventListener("click",e=>{e.preventDefault(),S()}),window.addEventListener("beforeunload",e=>{i.reportDirty&&(e.preventDefault(),e.returnValue="")}),!i.sessionId){h(s("page.consultation.notFound"));return}if(window.AstroAPI?.getAccountPreferences)try{window.accountPreferencesCache=await window.AstroAPI.getAccountPreferences(),window.AstroPreferences?.setAccountVisualPreferences?.(window.accountPreferencesCache?.visual||{})}catch(e){console.warn("Consultation account preferences fallback to defaults:",e)}await q(),document.getElementById("pageLoader")?.classList.add("hidden")}function S(){i.personId?window.location.href=`/client/${encodeURIComponent(i.personId)}`:window.history.back()}var P=["en","uk","ru","de"];async function q(){try{let t=await u(`${d}/call-sessions/${i.sessionId}`);if(t.status===404){h(s("page.consultation.notFound"));return}if(!t.ok)throw new Error(s("page.consultation.requestFailed",{status:t.status}));i.cs=await t.json(),i.personId=i.cs.person_id;let e=window.FrontendI18n?.normalizeLocale?.(i.cs.summary_json?.session?.language);if(e&&P.includes(e)&&window.FrontendI18n?.setLocale)try{await window.FrontendI18n.setLocale(e,{persist:!1,source:"consultation-language"})}catch{}C()}catch(t){h(t.message)}}function h(t){document.getElementById("pageLoader")?.classList.add("hidden"),document.getElementById("consultRoot").innerHTML=`<div class="ui-card"><p class="consult-empty">${n(t)}</p></div>`}function C(){let t=i.cs,e=t.summary_json,a=document.getElementById("consultRoot"),o=t.started_at?$(`${t.started_at}Z`):t.created_at?$(`${t.created_at}Z`):"",r="",c=(e?.session?.consultation_types||[]).map(_=>`<span class="consult-type-chip">${n(p("consultationType",_))}</span>`).join(""),f=`${o}${t.client_name?` · ${t.client_name}`:""}`;r+=`<div class="consult-head">
    ...
    </div>`;let m="",v="";t.call_status==="summary_failed"?m+=`<div class="ui-card consult-card-status">
            <p class="consult-empty">${n(s("page.consultation.summaryFailed"))}</p>
            ${t.summary_error?`<p class="consult-empty">${n(t.summary_error)}</p>`:""}
        </div>`:e?(m+=D(e),m+=F(t,e),v+=M(e.key_points||[]),v+=R(e.open_questions_or_unclear_items||[])):m+=H(t),v+=`<div class="ui-card consult-card-memory">
    ...
    </div>`}function F(t,e){let a=t.client_report_edited!=null?t.client_report_edited:e.client_facing_report?.text||"",o=t.client_report_shared_at?`<span class="consult-shared-note">${n(s("page.consultation.report.sharedAt"))} ${n($(`${t.client_report_shared_at}Z`))}</span>`:"";return`<div class="ui-card consult-card-report">
        <div class="consult-card-title">
    ...
    </div>`}function M(t){if(!t.length)return"";let e={};t.forEach(o=>{(e[o.category]=e[o.category]||[]).push(o)});let a="";return x.forEach(o=>{if(!e[o])return;let r=e[o].map(c=>`<li>${n(c.text)}<span class="consult-kp-by">${n(p("mentionedBy",c.mentioned_by))}</span></li>`).join("");a+=`<div class="consult-kp-group">
        ...
        ${a}
    </div>`}function R(t){if(!t.length)return`<div class="ui-card consult-card-questions">
        ...
        <ul class="consult-oq-list">${e}</ul>
    </div>`}function j(){let t=document.getElementById("reportArea"),e=document.getElementById("saveReportBtn"),a=document.getElementById("copyReportBtn");t&&(t.addEventListener("input",()=>{i.reportDirty=!0,e.disabled=!1,document.getElementById("reportStatus").innerHTML=`<span class="consult-dirty">${n(s("page.consultation.report.unsaved"))}</span>`}),e.addEventListener("click",async()=>{e.disabled=!0;try{let o=await u(`${d}/call-sessions/${i.sessionId}/report`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:t.value})});if(!o.ok)throw new Error(s("page.consultation.requestFailed",{status:o.status}));i.reportDirty=!1,document.getElementById("reportStatus").innerHTML=`<span class="consult-saved">${n(s("page.consultation.report.saved"))}</span>`}catch{e.disabled=!1,l(s("page.consultation.report.saveFailed"),"error")}}),a.addEventListener("click",async()=>{try{await navigator.clipboard.writeText(t.value),l(s("page.consultation.report.copied"),"success"),await u(`${d}/call-sessions/${i.sessionId}/report/shared`,{method:"POST"})}catch{l(s("page.consultation.report.copyFailed"),"error")}}))}async function g(){let t=document.getElementById("memList");if(!(!t||!i.userId))try{let e=await u(`${d}/clients/${i.userId}/memory`);if(!e.ok)throw new Error(s("page.consultation.requestFailed",{status:e.status}));let a=await e.json();O(a.entries||[])}catch{t.innerHTML=`<p class="consult-empty">${n(s("page.consultation.memory.loadError"))}</p>`}}function O(t){let e=document.getElementById("memList");if(!t.length){e.innerHTML=`<div class="ui-empty-state"><div class="ui-empty-state__icon" aria-hidden="true">✎</div><p class="ui-empty-state__text">${n(s("page.consultation.memory.empty"))}</p></div>`;return}e.innerHTML=t.map(a=>{let o=a.source==="ai"?`<span class="mem-provenance is-ai">✦ ${n(s("page.consultation.memory.aiSuggested"))}</span>`:`<span class="mem-provenance">${n(s("page.consultation.memory.byAstrologer"))}</span>`;return`<div class="mem-item" data-id="${n(a.id)}">
            ...
            <div class="mem-actions">
                <button class="ui-btn ui-btn--secondary ui-btn--sm" data-act="edit">${n(s("common.edit"))}</button>
        ...
        </div>`}).join(""),e.querySelectorAll(".mem-item").forEach(a=>{let o=a.dataset.id;a.querySelector('[data-act="edit"]')?.addEventListener("click",()=>N(a,o)),a.querySelector('[data-act="del"]')?.addEventListener("click",()=>Q(o))})}function N(t,e){let a=t.querySelector(".mem-text"),o=a.textContent,r=t.querySelector(".mem-actions");a.outerHTML=`<textarea class="mem-edit-area">${n(o)}</textarea>`,r.innerHTML=`
        <button class="ui-btn ui-btn--primary ui-btn--sm" data-act="savee">${n(s("common.save"))}</button>
        <button class="ui-btn ui-btn--ghost ui-btn--sm" data-act="cancel">${n(s("common.cancel"))}</button>`,r.querySelector('[data-act="savee"]').addEventListener("click",async()=>{let c=t.querySelector(".mem-edit-area").value.trim();if(c)try{if(!(await u(`${d}/memory/${e}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:c})})).ok)throw new Error;g()}catch{l(s("common.error"),"error")}}),r.querySelector('[data-act="cancel"]').addEventListener("click",g)}async function Q(t){if(confirm(s("page.consultation.memory.confirmDelete")))try{if(!(await u(`${d}/memory/${t}`,{method:"DELETE"})).ok)throw new Error;l(s("page.consultation.memory.deleted"),"success"),g()}catch{l(s("common.error"),"error")}}
