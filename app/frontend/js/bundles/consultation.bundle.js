import{a as I}from"./chunks/chunk-XUHCC6CH.js";import"./chunks/chunk-MZ7NETWD.js";import{b as y,d as _,e as b,f as E,g as L}from"./chunks/chunk-6NFCP3QL.js";var Q=y(_()),U=y(b()),Z=y(E());var K=y(L()),V=y(I());var u=window.location.hostname==="localhost"?"http://localhost:8000/api/v1":"/api/v1",T=["client_question","life_context","astrological_focus","guidance_given","decision_or_plan","follow_up","sensitive"],c={sessionId:null,userId:null,cs:null,reportDirty:!1};function s(t,e){return window.FrontendI18n?.t?.(t,e)||t}function k(t={}){return window.AstroAPI?.withLocaleHeaders?window.AstroAPI.withLocaleHeaders(t):t}function m(t,e={}){return fetch(t,{credentials:"include",...e,headers:k(e.headers||{})})}function n(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function f(t){if(!t)return"";let e=new Date(t);return Number.isNaN(e.getTime())?String(t):window.LocaleFormatters?.formatDateTime?window.LocaleFormatters.formatDateTime(e):e.toLocaleString()}function p(t,e){if(!e)return"";let o=`page.consultation.${t}.${e}`,a=s(o);return a===o?e:a}var w=null;function d(t,e=""){let o=document.getElementById("toast");o&&(o.textContent=t,o.className=`toast visible ${e}`,clearTimeout(w),w=setTimeout(()=>{o.className="toast"},2600))}function x(){let t=window.location.pathname.match(/\/consultation\/([^/?#]+)/);return t?decodeURIComponent(t[1]):null}document.addEventListener("DOMContentLoaded",P);async function P(){c.sessionId=x();let t=document.getElementById("backLink");if(t&&t.addEventListener("click",e=>{e.preventDefault(),A()}),window.addEventListener("beforeunload",e=>{c.reportDirty&&(e.preventDefault(),e.returnValue="")}),!c.sessionId){$(s("page.consultation.notFound"));return}if(window.AstroAPI?.getAccountPreferences)try{window.accountPreferencesCache=await window.AstroAPI.getAccountPreferences(),window.AstroPreferences?.setAccountVisualPreferences?.(window.accountPreferencesCache?.visual||{})}catch(e){console.warn("Consultation account preferences fallback to defaults:",e)}await S(),document.getElementById("pageLoader")?.classList.add("hidden")}function A(){c.userId?window.location.href=`/client/${encodeURIComponent(c.userId)}`:window.history.back()}var B=["en","uk","ru"];async function S(){try{let t=await m(`${u}/call-sessions/${c.sessionId}`);if(t.status===404){$(s("page.consultation.notFound"));return}if(!t.ok)throw new Error(`HTTP ${t.status}`);c.cs=await t.json(),c.userId=c.cs.user_id;let e=c.cs.summary_json?.session?.language;if(e&&B.includes(e)&&window.FrontendI18n?.setLocale)try{await window.FrontendI18n.setLocale(e,{persist:!1,source:"consultation-language"})}catch{}H()}catch(t){$(t.message)}}function $(t){document.getElementById("pageLoader")?.classList.add("hidden"),document.getElementById("consultRoot").innerHTML=`<div class="consult-card"><p class="consult-empty">${n(t)}</p></div>`}function H(){let t=c.cs,e=t.summary_json,o=document.getElementById("consultRoot"),a=t.started_at?f(`${t.started_at}Z`):t.created_at?f(`${t.created_at}Z`):"",r="",i=(e?.session?.consultation_types||[]).map(h=>`<span class="consult-type-chip">${n(p("consultationType",h))}</span>`).join("");r+=`<div class="consult-head">
    ...
    <p class="consult-meta">${n(a)}${t.client_name?" · "+n(t.client_name):""}</p>`;let l="",v="";t.call_status==="summary_failed"?l+=`<div class="consult-card consult-card-status">
        ...
        </div>`:e?(l+=q(e),l+=D(t,e),v+=R(e.key_points||[]),v+=F(e.open_questions_or_unclear_items||[])):l+=C(t),v+=`<div class="consult-card consult-card-memory">
    ...
    </div>`}function D(t,e){let o=t.client_report_edited!=null?t.client_report_edited:e.client_facing_report?.text||"",a=t.client_report_shared_at?`<span class="consult-shared-note">${n(s("page.consultation.report.sharedAt"))} ${n(f(`${t.client_report_shared_at}Z`))}</span>`:"";return`<div class="consult-card consult-card-report">
        <div class="consult-card-title">
    ...
    </div>`}function R(t){if(!t.length)return"";let e={};t.forEach(a=>{(e[a.category]=e[a.category]||[]).push(a)});let o="";return T.forEach(a=>{if(!e[a])return;let r=e[a].map(i=>`<li>${n(i.text)}<span class="consult-kp-by">${n(p("mentionedBy",i.mentioned_by))}</span></li>`).join("");o+=`<div class="consult-kp-group">
