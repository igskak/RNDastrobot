import{a as tt}from"./chunks/chunk-G5NALNVK.js";import"./chunks/chunk-YBGVRB7X.js";import{b as R,d as Qe,e as Xe,f as Ze,g as et}from"./chunks/chunk-AKKGPULT.js";var at=R(Qe()),st=R(Xe()),ct=R(Ze());var it=R(et()),lt=R(tt());(function(){"use strict";let J=["natal","biwheel","solar"],ee=window.AstroPreferences?.DEFAULT_ENABLED_ASPECT_TYPES||["Conjunction","Opposition","Trine","Square","Sextile","Vigintile","Semi_Nonagon","Semisextile","Decile","Nonagon","Semisquare","Quintile","Binonagon","Sentagon","Tridecile","Sesquiquadrate","Biquintile","Quincunx"],U=window.AstroPreferences?.ORB_PROFILE_IDS||["natal","prognostic"],F=window.AstroPreferences?.DEFAULT_ORB_PAIR_STRATEGY||"larger",Y="activePreferenceRecalcJobId",te="accountOrbViewMode",we=new Set(["TrueNode","SouthNode","BlackMoon","WhiteMoon","PartOfFortune","ASC","DSC","MC","IC","Vertex","AntiVertex"]),y=null,ne=null,h=null,oe=null,I=null,f="natal",C="default",q=null;function ae(){if(window.AstroAPI?.hidePageLoader){window.AstroAPI.hidePageLoader();return}let e=document.getElementById("pageLoader");e&&(e.classList.add("fade-out"),setTimeout(()=>e.remove(),460))}function b(e,n){return window.FrontendI18n?.t?.(e,n)||e}function _e(){let e=document.querySelector(".account-settings-back");if(!e)return;let n=window.AstroAPI?.getNavigationState?.()||{};e.href=n.sourceUrl||"/"}function i(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function $(e,n=""){let t=b(e);return t&&t!==e?t:n}function p(e){return $(`astro.planet.${e}`,window.Symbols?.getPlanetNameRu?.(e)||e)}function se(e){return window.Symbols?.getPlanetSymbol?.(e)||String(e||"").slice(0,2)||"•"}function L(e,n={}){return window.Symbols?.getPlanetSymbolMarkup?.(e,n)||`<span class="astro-symbol" aria-hidden="true">${i(se(e))}</span>`}function Ae(e){let n=window.AstroPreferences?.normalizeMatrixBodyName?window.AstroPreferences.normalizeMatrixBodyName(e):e,t=h?.default_dignities?.signs||{},o=window.Symbols?.signElements||{},a=V().map(s=>s?.name).filter(Boolean),c=a.length?a:Object.keys(t);for(let s of c)if(t?.[s]?.ruler===n)return o[s]||null;for(let s of c)if(t?.[s]?.co_ruler===n)return o[s]||null;for(let s of c)if(t?.[s]?.exaltation===n)return o[s]||null;return null}function Ee(e,n={}){let t=x(n),o=Ae(e);return window.AstroPreferences?.getPlanetColor?window.AstroPreferences.getPlanetColor(e,o,t):window.AstroPreferences?.getElementColor?window.AstroPreferences.getElementColor(o,t):"#6b7280"}function O(e){return $(`astro.aspect.${e}`,e)}function G(e){return window.Symbols?.getAspectDisplay?.(e)||window.Symbols?.aspects?.[e]||String(e||"").slice(0,3)||"•"}function Be(e,n){return window.AstroPreferences?.deepEqual?window.AstroPreferences.deepEqual(e,n):JSON.stringify(e??null)===JSON.stringify(n??null)}function ve(e){return e==null?e:JSON.parse(JSON.stringify(e))}function P(e={}){return window.AstroPreferences?.normalizeViewSettings?window.AstroPreferences.normalizeViewSettings(e):e}function _(e={}){return window.AstroPreferences?.normalizeMethodologySettings?window.AstroPreferences.normalizeMethodologySettings(e):e}function D(e={},n={}){return window.AstroPreferences?.normalizeDignitySettings?window.AstroPreferences.normalizeDignitySettings(e,n):e}function x(e={}){return window.AstroPreferences?.resolveVisualPreferences?window.AstroPreferences.resolveVisualPreferences(e):e||{}}function ce(e={}){return window.AstroPreferences?.ensureMatrixRows?window.AstroPreferences.ensureMatrixRows(e||{}):e||{}}function $e(){return window.AstroPreferences?.MATRIX_BODIES||[]}function M(){return h?.aspect_types||ee.map(e=>({aspect_type:e}))}function nt(e){return M().find(n=>n?.aspect_type===e)||null}function k(){return(h?.bodies||[]).map(e=>e?.name).filter(Boolean)}function V(){return h?.signs||[]}function re(){return k().filter(e=>!we.has(e))}function T(e="natal"){let n=M(),t=k();return window.AstroPreferences?.buildDefaultOrbProfileMatrix?window.AstroPreferences.buildDefaultOrbProfileMatrix(n,t,e):Object.fromEntries(n.map(o=>[o.aspect_type,Object.fromEntries(t.map(a=>[a,e==="prognostic"?a==="Moon"?3:1:Number(o.base_orb||5)]))]))}function v(){let e={version:2,pair_strategy:F,profiles:Object.fromEntries(U.map(n=>[n,{matrix:T(n)}]))};return _({orbs:e,balances:h?.default_balance_targets||{},dignities:h?.default_dignities||{version:1,signs:{}}})}function ie(){return x(h?.default_visual_palettes||{})}function le(){return{chart_defaults:{natal:P({}),biwheel:P({aspects:{scope:"major"}}),solar:P({})},chart_creation_defaults:{house_system:"P"},methodology:v(),visual:ie()}}function de(){return document.getElementById("accountTimezoneLabelFormatSelect")}function ue(){return document.getElementById("accountDateFormatSelect")}function me(){return document.getElementById("accountDegreeFormatSelect")}function ge(e){return e==="natal"?{orientation:document.getElementById("natalOrientationSelect"),aspectScope:document.getElementById("natalAspectScopeSelect"),showApplyingSeparating:document.getElementById("natalShowApplyingSeparating"),showSpeed:document.getElementById("natalShowSpeed"),showStationary:document.getElementById("natalShowStationary"),showAspectText:document.getElementById("natalShowAspectText")}:e==="biwheel"?{orientation:document.getElementById("biwheelOrientationSelectAccount"),aspectScope:document.getElementById("biwheelAspectScopeSelectAccount"),showApplyingSeparating:null,showSpeed:null,showStationary:null,showAspectText:document.getElementById("biwheelShowAspectTextAccount")}:{orientation:document.getElementById("solarOrientationSelectAccount"),aspectScope:document.getElementById("solarAspectScopeSelectAccount"),showApplyingSeparating:document.getElementById("solarShowApplyingSeparatingAccount"),showSpeed:document.getElementById("solarShowSpeedAccount"),showStationary:document.getElementById("solarShowStationaryAccount"),showAspectText:document.getElementById("solarShowAspectTextAccount")}}function A(){return y||(y={methodology:v()}),y.methodology=_(y.methodology||v()),y.methodology}function Pe(e){return A()?.orbs?.profiles?.[e]?.matrix||T(e)}function xe(){let e=document.getElementById("accountOrbPairStrategySelect");return e?window.AstroPreferences?.normalizeOrbPairStrategy?.(e.value)||F:_(y?.methodology||v())?.orbs?.pair_strategy||F}function pe(e){return $(`astro.sign.${e}`,window.Symbols?.signNamesRu?.[e]||e)}function Me(e){return window.Symbols?.signs?.[e]||String(e||"").slice(0,2)||"•"}function Te(e){let n=i(pe(e)),t=i(Me(e));return`<span class="astro-symbol" aria-hidden="true" title="${n}">${t}</span>`}function ot(e){return V().find(n=>n?.name===e)?.opposite||null}function W(){let e=A();return e.dignities=D(e.dignities||{},h?.default_dignities||{}),e.dignities}function Ie(e={}){let n=re(),t=Object.fromEntries(n.map(o=>[o,{domicile_primary:[],domicile_secondary:[],detriment_primary:[],detriment_secondary:[],exaltation:[],fall:[]}]));return V().forEach(o=>{let a=o?.name,c=o?.opposite,s=e?.signs?.[a]||{};s.ruler&&t[s.ruler]&&(t[s.ruler].domicile_primary.push(a),c&&t[s.ruler].detriment_primary.push(c)),s.co_ruler&&t[s.co_ruler]&&(t[s.co_ruler].domicile_secondary.push(a),c&&t[s.co_ruler].detriment_secondary.push(c)),s.exaltation&&t[s.exaltation]&&(t[s.exaltation].exaltation.push(a),c&&t[s.exaltation].fall.push(c))}),t}function z(e=[],{mode:n="derived",secondarySigns:t=[]}={}){let o=new Set(t||[]);return V().map(a=>{let c=a?.name,s=e.includes(c)||o.has(c),r=o.has(c),u=["account-settings-dignity-glyph",s?"is-active":"",r?"is-secondary":"",n==="derived"?"is-derived":""].filter(Boolean).join(" "),l=i(pe(c)),d=i(`${l} · ${b(s?r?"page.accountSettings.dignities.states.secondary":"page.accountSettings.dignities.states.primary":"page.accountSettings.dignities.states.empty")}`);return`
                <button
                    ...
                    ${n==="derived"?"disabled":""}
                >${Te(c)}</button>
            `}).join("")}function fe(){let e=document.getElementById("accountOrbProfileHint"),n=document.getElementById("accountOrbMatrixPanel"),t=document.getElementById("accountApplyNatalOrbsBtn");document.querySelectorAll("[data-orb-profile-tab]").forEach(o=>{let a=o.dataset.orbProfileTab===f;o.classList.toggle("is-active",a),o.setAttribute("aria-selected",a?"true":"false"),n&&a&&o.id&&n.setAttribute("aria-labelledby",o.id)}),e&&(e.textContent=b(`page.accountSettings.orbs.hints.${f}`)),t&&t.classList.toggle("hidden",f!=="prognostic")}function ye(){let e=document.getElementById("accountOrbMatrixPanel"),n=document.getElementById("accountOrbsTable");document.querySelectorAll("[data-orb-view-mode]").forEach(t=>{let o=t.dataset.orbViewMode===C;t.classList.toggle("is-active",o),t.setAttribute("aria-selected",o?"true":"false")}),e?.classList.toggle("is-compact",C==="compact"),n?.classList.toggle("is-compact",C==="compact")}function K(){let e=A(),n=T(f);document.querySelectorAll("#accountOrbsMatrixBody input[data-orb-aspect-type][data-orb-body]").forEach(t=>{let o=t.dataset.orbAspectType,a=t.dataset.orbBody;!o||!a||(n[o]||(n[o]={}),n[o][a]=Number.parseFloat(t.value)||0)}),e.orbs.profiles[f]={matrix:n}}function Ce(e,{rerender:n=!0}={}){U.includes(e)&&(y&&K(),f=e,fe(),n&&y&&Q(y.methodology))}function Le(e){["default","compact"].includes(e)&&(C=e,localStorage.setItem(te,e),ye())}function Oe(e={}){let n=document.getElementById("accountAspectTypesMatrixBody");n&&(n.innerHTML=M().map(t=>{let o=t.aspect_type,a=i(G(o)),c=i(O(o)),s=J.map(r=>{let u=e?.[r]?.aspects?.enabled_types||[],g=new Set(Array.isArray(u)&&u.length?u:ee).has(o)?"checked":"";return`
                    <td>
            ...
            `}).join(""))}function De(e={}){let n=document.getElementById("accountBodiesMatrixBody");n&&(n.innerHTML=$e().map(t=>{let o=i(p(t)),a=L(t,{size:18,title:p(t)}),c=J.map(s=>{let r=ce(e?.[s]?.matrix?.rows||{}),u=r?.[t]?.display!==!1?"checked":"",l=r?.[t]?.aspecting!==!1?"checked":"";return`
                    <td>
                        <label class="account-settings-matrix-toggle">
                            <input
                                ...
                                ${u}
                                aria-label="${i(`${$(`page.accountSettings.tables.columns.${s}`,s)}: ${o} ${b("page.accountSettings.matrix.columns.display")}`)}"
                            >
                        </label>
                    </td>
                    <td>
                        <label class="account-settings-matrix-toggle">
                            <input
                                ...
                                ${l}
                                aria-label="${i(`${$(`page.accountSettings.tables.columns.${s}`,s)}: ${o} ${b("page.accountSettings.matrix.columns.aspecting")}`)}"
                            >
                        </label>
                    </td>
                `}).join("");return`
            ...
            `}).join(""))}function Q(e={}){let n=document.getElementById("accountOrbsHeaderRow"),t=document.getElementById("accountOrbsMatrixBody");if(!n||!t)return;let o=k(),a=M(),s=_(e||v())?.orbs?.profiles?.[f]?.matrix||T(f);n.innerHTML=`
        ...
        `,t.innerHTML=a.map(r=>{let u=r.aspect_type,l=i(G(u)),g=i(O(u));return`
            ...
            `}).join(""),fe(),ye()}function be(e={}){let n=document.getElementById("accountDignitiesMatrixBody");if(!n)return;let t=D(e?.dignities||{},h?.default_dignities||{}),o=Ie(t);n.innerHTML=re().map(a=>{let c=i(p(a)),s=o[a]||{domicile_primary:[],domicile_secondary:[],detriment_primary:[],detriment_secondary:[],exaltation:[],fall:[]};return`
                <tr>
        ...
        `).join("")}function Fe(e={}){let n=document.getElementById("accountAspectColorsBody");if(!n)return;let t=x(e);n.innerHTML=M().map(o=>{let a=o.aspect_type,c=window.AstroPreferences?.getAspectColor?window.AstroPreferences.getAspectColor(a,t):"#9ca3af";return`
                <tr>
                    <th scope="row" class="account-settings-icon-cell">
                        <span class="account-settings-aspect-meta account-settings-aspect-meta--icon-only">
        ...
        `).join(""),t.innerHTML=k().map(r=>{let u=Ee(r,o),l=!!c?.[r],g=c?.[r]||u;return`
                <tr>
                    <th scope="row" class="account-settings-icon-cell">
                        <span class="account-settings-body account-settings-body--icon-only">
        ...
        `,t.classList.remove("hidden"),t.dataset.status=s}function Z(){I&&(clearTimeout(I),I=null)}async function Se(e){if(Z(),!e||!window.AstroAPI?.getPreferenceRecalcJob)return;sessionStorage.setItem(Y,String(e));let n=async()=>{try{let t=await window.AstroAPI.getPreferenceRecalcJob(e);if(H(t,{final:t.status==="completed"||t.status==="failed"}),t.status==="completed"){sessionStorage.removeItem(Y),E(t.failed_count?`Пересчет завершен с ошибками: ${t.failed_count}.`:"Пересчет карт завершен.",t.failed_count?"info":"success"),await Je(),Z();return}if(t.status==="failed"){sessionStorage.removeItem(Y),E(t.error||"Пересчет карт не выполнен.","error"),Z();return}I=setTimeout(n,2500)}catch(t){I=setTimeout(n,4e3),console.warn("Failed to poll preference recalculation job:",t)}};await n()}async function Ue(){let e=await window.AstroAPI?.requireAuth?.({redirectTo:"/login.html"});if(!e)return;let n=document.getElementById("accountSettingsSubtitle");n&&(n.textContent=e.email?b("page.accountSettings.subtitleWithEmail",{email:e.email}):b("page.accountSettings.subtitle"));let[t,o]=await Promise.all([window.AstroAPI.getPreferencesMetadata?.(),window.AstroAPI.getAccountPreferences()]);h=t||null,j(o,{updateBaseline:!0});let a=sessionStorage.getItem(Y);a?Se(a).catch(c=>{console.warn("Failed to resume recalculation job polling:",c)}):H(null),ae()}async function Ge(){let e=document.getElementById("saveAccountSettingsBtn");e&&(e.disabled=!0);try{let n=He(),t=localStorage.getItem("currentUserId")||null,o=!Be(_(ne||{}),n.methodology),a=await window.AstroAPI.patchAccountPreferences(n);if(j(a,{updateBaseline:!0}),o&&window.AstroAPI?.createPreferenceRecalcJob){let c=await window.AstroAPI.createPreferenceRecalcJob({job_type:"methodology_recalc",payload:{source:"account-settings",...t?{priority_user_id:t}:{}}});H(c),Se(c.job_id).catch(s=>{console.warn("Failed to poll methodology recalculation job:",s)}),E("Настройки сохранены. Карты пересчитываются с учетом новых настроек.","success"),requestAnimationFrame(he);return}E(b("page.accountSettings.toasts.saved"),"success"),requestAnimationFrame(he)}catch(n){E(n.message||b("page.accountSettings.toasts.saveFailed"),"error")}finally{e&&(e.disabled=!1)}}function We(){j(le()),H(null),E(b("page.accountSettings.toasts.restored"),"info")}function N({restoreFocus:e=!0}={}){let n=document.getElementById("accountSettingsResetConfirmDialog"),t=document.getElementById("accountSettingsResetConfirmBackdrop");n&&n.classList.add("hidden"),t&&t.classList.add("hidden"),document.body.classList.remove("account-settings-modal-open"),e&&q instanceof HTMLElement&&q.focus(),q=null}function Ke(){let e=document.getElementById("accountSettingsResetConfirmDialog"),n=document.getElementById("accountSettingsResetConfirmBackdrop"),t=document.getElementById("accountSettingsResetConfirmSubmit");!e||!n||(q=document.activeElement instanceof HTMLElement?document.activeElement:null,n.classList.remove("hidden"),e.classList.remove("hidden"),document.body.classList.add("account-settings-modal-open"),requestAnimationFrame(()=>{t?.focus()}))}document.addEventListener("DOMContentLoaded",async()=>{C=localStorage.getItem(te)==="compact"?"compact":"default",_e();let e=document.getElementById("saveAccountSettingsBtn"),n=document.getElementById("restoreStandardDefaultsBtn"),t=document.getElementById("accountApplyNatalOrbsBtn"),o=document.getElementById("accountOrbsMatrixBody"),a=document.getElementById("accountDignitiesMatrixBody"),c=document.getElementById("accountBodyOverrideColorsBody"),s=document.getElementById("accountSettingsResetConfirmDialog"),r=document.getElementById("accountSettingsResetConfirmBackdrop"),u=document.getElementById("accountSettingsResetConfirmClose"),l=document.getElementById("accountSettingsResetConfirmCancel"),g=document.getElementById("accountSettingsResetConfirmSubmit");e?.addEventListener("click",()=>{Ge()}),n?.addEventListener("click",()=>{Ke()}),r?.addEventListener("click",()=>{N()}),u?.addEventListener("click",()=>{N()}),l?.addEventListener("click",()=>{N()}),g?.addEventListener("click",()=>{N({restoreFocus:!1}),We()}),document.querySelectorAll("[data-orb-profile-tab]").forEach(d=>{d.addEventListener("click",()=>{Ce(d.dataset.orbProfileTab||"natal")})}),document.querySelectorAll("[data-orb-view-mode]").forEach(d=>{d.addEventListener("click",()=>{Le(d.dataset.orbViewMode||"default")})}),t?.addEventListener("click",()=>{K();let d=A();d.orbs.profiles.prognostic={matrix:JSON.parse(JSON.stringify(Pe("natal")))},f="prognostic",Q(d),E(b("page.accountSettings.toasts.orbsCopied"),"info")}),o?.addEventListener("input",d=>{let m=d.target;if(!(m instanceof HTMLInputElement)||!m.dataset.orbAspectType||!m.dataset.orbBody)return;let S=A(),B=(S.orbs.profiles[f]||{matrix:T(f)}).matrix||T(f);B[m.dataset.orbAspectType]||(B[m.dataset.orbAspectType]={}),B[m.dataset.orbAspectType][m.dataset.orbBody]=Number.parseFloat(m.value)||0,S.orbs.profiles[f]={matrix:B}}),a?.addEventListener("click",d=>{let m=d.target.closest("[data-dignity-mode][data-dignity-sign]");if(!(m instanceof HTMLButtonElement))return;let S=m.dataset.dignityMode,w=m.dataset.dignitySign,B=m.closest("[data-dignity-planet]")?.dataset?.dignityPlanet;!S||!w||!B||S==="derived"||(S==="domicile"?ke(w,B):S==="exaltation"&&Ne(w,B),be(y?.methodology||A()))}),c?.addEventListener("input",d=>{let m=d.target;if(!(m instanceof HTMLInputElement)||!m.dataset.bodyColorOverride)return;m.dataset.bodyColorActive="true",c.querySelector(`[data-clear-body-color-override="${m.dataset.bodyColorOverride}"]`)?.classList.remove("is-muted")}),c?.addEventListener("click",d=>{let m=d.target.closest("[data-clear-body-color-override]");if(!(m instanceof HTMLElement))return;let S=m.dataset.clearBodyColorOverride;if(!S)return;let w=c.querySelector(`[data-body-color-override="${S}"]`);w instanceof HTMLInputElement&&(w.dataset.bodyColorActive="false",w.value=w.dataset.bodyColorDefault||"#6b7280",m.classList.add("is-muted"))});try{await window.FrontendI18n?.ready?.catch?.(()=>{}),await Ue(),document.addEventListener("frontend:locale-changed",()=>{y&&j(y)}),document.addEventListener("keydown",d=>{d.key==="Escape"&&(!s||s.classList.contains("hidden")||N())})}catch(d){E(d.message||b("page.accountSettings.toasts.loadFailed"),"error"),ae()}})})();
