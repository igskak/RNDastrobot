...
from app.api.routes import auth, natal, transits, solar, progressions, directions, ingresses, places, consultations, alerts, preferences, call_sessions, synastry, charts, chart_imports, persons, assistant, billing, lunar, electional, composite, forecast, profections, antiscia, asteroids, dominants, fixed_stars, declination, client_memory, conversions, ops
...
# English is served from the bare URLs; every other locale gets its own prerendered
# document under a path prefix (/de/pricing.html), generated by
# app/scripts/build-localized-pages.mjs. Which pages exist per locale is read off disk,
# so the build script alone decides what is translated.
PREFIXED_LOCALES = tuple(loc for loc in SUPPORTED_LOCALES if loc != DEFAULT_LOCALE)

# Bare URLs that have a translated counterpart. Used to send legacy ?lang= links to the
# prefixed URL so one page is not indexed under two addresses.
LOCALIZABLE_BARE_PATHS = ("/", "/index.html", "/pricing.html", "/terms.html", "/login.html")


def localized_document_path(locale: str, page: str) -> Optional[str]:
    """Absolute path of a prerendered page, or None when it is not available.

    The page name is validated rather than joined blindly: this value comes straight
    from the URL.
    """
    if locale not in PREFIXED_LOCALES:
        return None

    page = page or "index.html"
    if not re.fullmatch(r"[A-Za-z0-9._-]+\.html", page):
        return None

    locale_root = os.path.join(FRONTEND_PATH, locale)
    candidate = os.path.normpath(os.path.join(locale_root, page))
    if os.path.dirname(candidate) != locale_root or not os.path.exists(candidate):
        return None

    return candidate


def localized_path_for(path: str, locale: str) -> str:
    """Prefix a bare site path with a locale ('/pricing.html' -> '/de/pricing.html')."""
    if locale not in PREFIXED_LOCALES:
        return path
    return f"/{locale}/" if path in ("/", "/index.html") else f"/{locale}{path}"
...
async def isolate_solo_marketing_routes(request: Request, call_next):
    ...
    commercial_paths |= {
        localized_path_for("/pricing.html", locale) for locale in PREFIXED_LOCALES
    }
...
async def redirect_legacy_locale_query(request: Request, call_next):
    ...
    Links built before the prefixed URLs existed (older ads, shared links) still render
    the right language, but a permanent redirect keeps one page from being indexed —
    and measured — under two addresses.
    """
    path = request.url.path
    if request.method in ("GET", "HEAD") and path in LOCALIZABLE_BARE_PATHS:
        params = dict(request.query_params)
        requested = normalize_locale(params.get("locale") or params.get("lang"))

        if requested in PREFIXED_LOCALES:
            params.pop("locale", None)
            params.pop("lang", None)
            target = localized_path_for(path, requested)
            # Everything else (gclid, utm_*, mode=register) has to survive the hop.
            if params:
...
app.include_router(chart_imports.router, prefix="/api/v1", tags=["Chart imports"])
...
def _sitemap_alternates(base: str, path: str) -> str:
    """hreflang block for a page that exists in several languages.

    Every URL in a translated set has to list the whole set, itself included, or search
    engines treat the pages as unrelated duplicates competing with each other.
    """
    rows = [("en", f"{base}{path}")]
    rows += [(locale, f"{base}{localized_path_for(path, locale)}") for locale in sorted(PREFIXED_LOCALES)]
    rows.append(("x-default", f"{base}{path}"))
    return "".join(
        f'<xhtml:link rel="alternate" hreflang="{hreflang}" href="{href}"/>'
        for hreflang, href in rows
    )


@app.get("/sitemap.xml")
async def sitemap_xml():
    """Sitemap of public, indexable pages, in every language they are published in."""
    base = _public_base_url()

    entries = []
    for path, priority in _SITEMAP_PAGES:
        if path in LOCALIZABLE_BARE_PATHS:
            alternates = _sitemap_alternates(base, path)
            locale_paths = [path] + [localized_path_for(path, locale) for locale in sorted(PREFIXED_LOCALES)]
        else:
            alternates = ""
            locale_paths = [path]
        ...
        for locale_path in locale_paths:
            entries.append(
                f"<url><loc>{base}{locale_path}</loc>"
...
languages: English, German, Ukrainian, Russian — German at {base}/de/, Russian at
{base}/ru/, Ukrainian at {base}/uk/. Not aimed at hobbyists wanting a one-off free
chart, and not a consumer horoscope product.
...
# Locale-prefixed documents are declared last: their path patterns are broad enough to
# shadow any route defined after them.
@app.get("/{locale}", include_in_schema=False)
async def localized_index_redirect(locale: str):
    """/de -> /de/, so the prerendered landing page has a single address."""
    if locale not in PREFIXED_LOCALES:
        raise HTTPException(status_code=404, detail="Page not found")
    return RedirectResponse(url=f"/{locale}/", status_code=308)


@app.get("/{locale}/", include_in_schema=False)
@app.get("/{locale}/{page}", include_in_schema=False)
async def localized_page(request: Request, locale: str, page: str = "index.html"):
    """Serve a prerendered public page for a non-default locale.

    A missing page 404s rather than falling back to English: silently serving another
    language would leave the visitor on a URL that promises one thing and shows another,
    and would let search engines index the same English page under every prefix.
    """
    if locale not in PREFIXED_LOCALES:
        raise HTTPException(status_code=404, detail="Page not found")
    ...
    # Mirrors the root route: a signed-in visitor wants their workspace, not the
    # marketing page. The app itself is localized from their stored preference.
    if page == "index.html" and request.cookies.get("astrobot_session"):
        clients_path = os.path.join(FRONTEND_PATH, "clients.html")
        if os.path.exists(clients_path):
            return FileResponse(clients_path)

    document = localized_document_path(locale, page)
    if document is None:
        raise HTTPException(status_code=404, detail="Page not found")
