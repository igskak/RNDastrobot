...
test('CI workflow wires i18n checkers before regression tests', () => {
    ...
    const usedKeysCmd = 'node app/scripts/check-i18n-used-keys.cjs';
    ...
    assert.ok(content.includes(usedKeysCmd), 'Used-key checker is not wired in CI');
    ...
    const usedKeysIndex = content.indexOf(usedKeysCmd);
    ...
    assert.ok(missingIndex >= 0 && usedKeysIndex >= 0 && hardcodedIndex >= 0 && untranslatedIndex >= 0, 'Checker commands not found');
