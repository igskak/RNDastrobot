...
**Статус на 2026-09-24:** 1, 2, 3, 4 и 8 исправлены и выложены на прод (`cc1bd86c`),
проверено вживую. В пункте 8 заодно убраны «recordings» у Practitioner из `/pricing.md` и
карточек тарифов на трёх conquest-страницах. Открыт вопрос: ассистент включён и на
Practitioner (`standard.assistant_enabled=True`), а страница цен обещает его только в
Studio — нужно решить, что верно. Слайды 2–4 пересняты после выкладки.
Пункт 5 сделан (`9491896d`: рабочий экран, ассистент и профиль на лендинге; у консультаций картинки нет до кадра записи звонка). Остаются 6 и 7.
...
| 2 | `gallery/02-profile.png` | Профиль человека: карты, заметки, контакт, партнёр | готово (переснято на проде после фиксов) |
| 3 | `gallery/03-assistant.png` | Ответ из данных карты: Плутон по Луне, 3 точных прохода | готово (переснято на проде после фиксов форматирования времени и подписей) |
...
Их оставить до запуска (могут понадобиться для кадра консультации и видео). После
запуска удалить. ID всех записей — в `gallery-src/demo-data.json`.

На кадрах списка скрыты старые тестовые строки аккаунта (кириллица). Остальное на
