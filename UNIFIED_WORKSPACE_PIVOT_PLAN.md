...
1. ✅ **Переключатель «Одно колесо / Мульти»** (коммит `28a5063`) — кнопки ◯/◎ в тулбаре, `setVisibleMethods(['natal'])`, персистится по карте, весь chrome жив в обоих видах.
2. ✅ **Соляр и синастрия как слои** (коммит `b1b16c9`) — тогглы в шапке (+год для соляра, +селект партнёра), кэш-ключи, persistence, generic правая панель рендерит их таблицы. Проверено: **5 колец одновременно** (натал+транзит+прогрессия+соляр+синастрия).
3. База = левая панель forecast-new как есть (quick-open, ручная правка момента, тикер, сохранение) — уже работало.
4. Скелет `/workspace` — тестбед. Редирект `/workspace` → forecast-new отложен до **cold-start пути** в forecast-new (сейчас он требует карту в сессии и редиректит на `/`; скелет покрывает сценарий «с нуля/вручную»). Это последний шаг Path B.
