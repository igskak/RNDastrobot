...
## ⚠️ Канонический хост для вебхуков и интеграций

Апекс `steliara.com` отдаёт **307-редирект на `www.steliara.com`** (правило
Cloudflare). Браузеры за редиректом идут, **а серверные интеграции (Stripe и др.
вебхуки) — нет**: они получают 307 и считают доставку проваленной.

**Правило:** все URL вебхуков и server-to-server интеграций ОБЯЗАНЫ использовать
канонический хост **`https://www.steliara.com/...`**, а не апекс.

Пример (Stripe billing webhook):
```
https://www.steliara.com/api/v1/webhooks/billing/stripe   ✅
https://steliara.com/api/v1/webhooks/billing/stripe       ❌ (307 → не доставится)
```

Это уже стоило одного «молчаливого» сбоя: первый Stripe-эндпоинт был на апексе,
события подписок не доходили, подписки не активировались. Если есть доступ к
Cloudflare — исключите `/api/*` из редиректа апекс→www, тогда оба хоста будут
работать. До тех пор — только `www`.
